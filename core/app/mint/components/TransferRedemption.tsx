...
import { useTransferRedemption } from "../hooks/transactions/useTransferRedemption";
import { useOption } from "../hooks/useOption";
import { Address, formatUnits, isAddress, parseUnits } from "viem";
import { useWaitForTransactionReceipt } from "wagmi";

interface TransferRedemptionProps {
  optionAddress: Address | undefined;
}

/**
 * Transfer Redemption tokens to another address
 */
export function TransferRedemption({ optionAddress }: TransferRedemptionProps) {
  ...
  const transferTx = useTransferRedemption();
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      // Transfer
      const hash = await transferTx.transfer(option.redemption, recipient as Address, wei);
      setTxHash(hash);
    } catch (err: any) {
      setStatus("error");
  ...
  if (!option) {
    return (
      <div className="p-6 bg-black/80 border border-gray-800 rounded-lg shadow-lg">
        <h2 className="text-xl font-light text-purple-300 mb-4">Transfer Redemptions</h2>
...
export default TransferRedemption;
