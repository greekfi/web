...
that are fully collateralized without requiring oracles or margin.
...
Similar to yield generating tokens, the protocol generates two ERC20 tokens:
a redemption token (which provides capital efficiency) and an option token.
We dive into the mechanics of these tokens and how they provide a new level of composability
to create a new options ecosystem that both encompasses and surpasses traditional financial options strategies.
...
\begin{itemize}
  ...
  \item Two tokens/contracts drive the protocol (OPTION and REDEMPTION) similar to yield platforms
...
The \greekfi protocol uses a contract factory, allowing upgradability, which creates a new pair (REDEMPTION and OPTION) of ERC20 contracts/tokens for each option type created (i.e. WETH/USDC, WBTC/USDC).
The pair token creation provides ERC20 composability in REDEMPTION and OPTION, a necessity in the adoption of options as tokens.
The pair provides a minting interface where depositing collateral tokens returns REDEMPTION and OPTION tokens.
...
The OPTION and REDEMPTION contracts are coupled to each other, meaning every interaction with the OPTION token will interact with the REDEMPTION token.
...
In order to exercise the OPTION option, the user's consideration is swapped for the collateral held by the REDEMPTION contract.
\begin{figure}[h]
  \centering
...
The OPTION token represents the long position of the option holder,
entitled to purchase the underlying collateral asset when exercising the option. 
Simply put, the OPTION token is the so-called "Option" in the traditional options world.

The OPTION token can only be exercised before expiration. After expiration, the OPTION token
is unusable and deemed worthless. We leave room to burn expired tokens so they aren't used for scams. 

The OPTION token can be traded on DEXs/AMMs and traded on RFQs.
They can also be used for OTC trades between private parties.

Other than standard ERC20 functionality, the OPTION token permits a user to \textbf{exercise} the option and
\textbf{redeem} the underlying collateral if and only if they hold the REDEMPTION token as well.
...
\hline
mint()  & Mints OPTION and REDEMPTION tokens by depositing collateral. \\
\hline
exercise() & Exercises the option by swapping consideration for collateral. \\
\hline
redeem() & Redeems collateral before expiration if also holding OPTION. \\
\hline
lock()/unlock() & Locks/unlocks to prevent/allow exercise. \\
\hline
\end{tabular}
\caption{Available functions for OPTION token}
\label{tab:option_functions}
\end{table}


\subsection{REDEMPTION Token}

The REDEMPTION token represents the short position of the option writer,
obligated to sell the underlying collateral asset for the consideration
asset when the OPTION option is exercised.
In TradFi, when an option is written and sold, the account shows a negative balance of the option. 
We represent this position with the REDEMPTION token.

\paragraph*{REDEMPTION is Put?} The REDEMPTION token is not a Put option. It is a short position in the option trade.
Being short does represent an expected negative outlook on the Collateral asset with respect to the Consideration asset,
which is similar to a Put option but they are not the same.



The REDEMPTION token also represents a long position in the underlying Collateral asset by locking up the underlying collateral asset in the REDEMPTION token.
...
The REDEMPTION token is also able to redeem the underlying collateral asset after expiration.

The REDEMPTION token provides an ability to redeem the underlying collateral asset after expiration.
Additionally, prior to expiration, the REDEMPTION token provides the ability to redeem the consideration asset after OPTION holders exercise.
And finally, if a REDEMPTION holder also owns the OPTION token, they can redeem the collateral prior to expiration (this is gamma neutral).
...
\hline
\end{tabular}
\caption{Available functions for REDEMPTION token}
\label{tab:functions}
\end{table}
...
One requirement for CE is for the REDEMPTION token to be used as collateral for loans for the underlying asset.
...
Below Strike (K), REDEMPTION is worth P, the price of the collateral.
Above Strike (K), REDEMPTION is worth only K because the consideration has been paid through exercise.
The chart below shows the price of the REDEMPTION token in those regimes with a Strike of 3000.
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{short_price.png}
  \caption{Price of the REDEMPTION token in the two regimes. The orange solid line shows the realistic price incorporating OPTION token value, while the green dot-dashed line shows an extreme fair price for low-volume assets.}
...
More importantly, the more realistic price of the REDEMPTION token incorporates the market price of the OPTION token, as seen as the orange curve solid line in the chart above.
Again, another consideration for the oracle, but for assets that have low volume, an extreme fair price can be used by really shifting the OPTION token price component as seen by the green dot-dashed line in the chart above.

\paragraph*{Margin Call Mechanics}
If the lender needs to margin call against the REDEMPTION token, there are two options: 1. The Lender can buy the coupled OPTION token, redeem the resulting collateral and either swap it or use it to pay back the loan. 2. If there's a liquid market for the REDEMPTION token, immediately sell it to cover the loan, with a possibility of using the proceeds to buy the borrowed collateral.
...
An Oracle or Third party can \textbf{unlock} the OPTION token to allow exercise.
...
The obvious solution is to use RFQs, such as 0x, Bebop, etc.
This allows
for the options to be traded at a price that is determined by the
market.
This would allow partnering with market makers to provide
...
The REDEMPTION token represents two things:
...
soon). When you sold your OPTION token you received a premium of 1000 USDC
in addition to IV and Time Value. This is potentially priced in the
REDEMPTION token making it tradable since it has a value.
...
of options with a portion of the collateral as the option premium.
A simple strategy could involve users depositing wETH as collateral, and
...
Using assets from a flash loan,
an OPTION can be exercised for collateral which is used to repay the loan.
